    ...
    use core_lib::tls_client::requests::task::{start_stream, run_task};
    ...
    async fn main() -> Result<(), Box<dyn std::error::Error>> {
...

        {
            ...
            while let Some(run) = stream.recv().await {
                if run.end_time.is_some() {
                    // Task skončil
                    println!(
                        "[Run {}] task={} DONE output={:?} return_code={:?}",
                        run.id, run.task_id, run.output, run.return_code
                    );
                } else {
                    // Priebežný update
                    println!(
                        "[Run {}] task={} RUNNING output={:?}",
                        run.id, run.task_id, run.output
                    );
                }
            }
        }
